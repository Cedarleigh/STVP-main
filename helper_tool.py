...
import nearest_neighbors.lib.python.nearest_neighbors as nearest_neighbors #.lib.python.nearest_neighbors


class Config:
    k_n = 16  # KNN
    num_layers = 5 # Number of layers
    num_points =12288# Number of input points 40960
    num_classes = 2 # Number of valid classes
    sub_grid_size = 0.04  # preprocess_parameter

    batch_size =8 # batch_size during training #4
    val_batch_size =8# batch_size during validation and test
    train_steps=1250 # Number of steps per epochs #2408
    val_steps =231# Number of validation steps per epoch #58

    sub_sampling_ratio = [4, 4, 4, 4, 2]  # sampling ratio of random sampling at each layer
    d_out = [16, 64, 128, 256, 512]  # feature dimension

    noise_init = 6  # noise initial parameter
...
class DataProcessing:
    ...
    def get_class_weights(dataset_name):
        # pre-calculate the number of points
        num_per_class = []
        if dataset_name == '8i':
            num_per_class = np.array([3,2], dtype=np.int32)#835394,225070,
...
class Plot:
    ...
    @staticmethod
    def draw_pc_sem_ins(pc_xyz, pc_sem_ins,name,plot_colors=None):
        """
        ...
        write_ply(join(' ',name),[Y_semins],['x', 'y', 'z', 'red', 'green', 'blue'])
