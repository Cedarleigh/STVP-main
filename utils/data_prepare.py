...
sub_grid_size = 0.04
original_pc_folder = os.path.join(dirname(dataset_path), 'original_ply')
sub_pc_folder =os.path.join(dirname(dataset_path), 'input_{:.3f}'.format(sub_grid_size))
os.mkdir(original_pc_folder) if not exists(original_pc_folder) else None
os.mkdir(sub_pc_folder) if not exists(sub_pc_folder) else None
out_format = '.ply'


def convert_pc2ply(anno_path, save_path,i):
    """
    Convert original dataset files to ply file (each line is XYZRGBL).
    :param anno_path: path to annotations.
...
if __name__ == '__main__':
    ...
    for i,annotation_path in enumerate( anno_paths):
        print(annotation_path)
        elements = str(annotation_path).split('\\')
#
        out_file_name = elements[-1]+"_now" + out_format
        convert_pc2ply(annotation_path,join(original_pc_folder, out_file_name),i)
